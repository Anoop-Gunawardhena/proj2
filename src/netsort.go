...
func ConnectToSockets(cType string,host string, port string,numServers int,clientindex int, Connchannel chan<- SocketCon){
	...
	if err != nil {
		...
		sendme := SocketCon{clientindex, conn}
...
func main() {
	...
	Connchannel := make(chan SocketCon)
	count := 0
	index2 := 0
	for {
		...
		capture := <-Connchannel
		Connections[capture.destination] = capture.conn
